...
namespace TPSBR
{
	public class ProjectileWeapon : FirearmWeapon
	{
        ...
        protected override bool FireProjectile(Vector3 firePosition, Vector3 targetPosition, Vector3 direction, float distanceToTarget, LayerMask hitMask, bool isFirst)
		{
			...
			var projectile = Runner.Spawn(_projectile, FireTransform.position/*firePosition*/, Quaternion.LookRotation(direction), Object.InputAuthority, BeforeProjectileSpawned, predictionKey);

			if (projectile == null)
				return false;

			projectile.Fire(Owner, FireTransform.position/*firePosition*/, direction * _projectileSpeed, hitMask, HitType);
