...
namespace TPSBR
{
	...
	public sealed class AgentInput : ContextBehaviour, IBeforeUpdate, IBeforeTick
	{
		...
		void IBeforeUpdate.BeforeUpdate()
		{
			...
			_renderInput.ToggleSide        = false; /*keyboard.eKey.isPressed*/
#if UNITY_EDITOR
		...
		private byte GetWeaponInput(Keyboard keyboard)
		{
            ...
            if (keyboard.digit3Key.wasPressedThisFrame == true) { weaponSlot = 0; }
            if (keyboard.digit2Key.wasPressedThisFrame == true) { weaponSlot = 1; }
            if (keyboard.digit1Key.wasPressedThisFrame == true) { weaponSlot = 2; }
