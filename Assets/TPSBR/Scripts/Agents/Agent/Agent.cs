...
namespace TPSBR
{
	...
	public sealed class Agent : ContextBehaviour
	{
		...
		private void ProcessFixedInput()
		{
			...
			// Added
			Vector2 newRecoil = Vector2.zero;
            ...
            if (_health.IsAlive == true)
			{
				...
				// Added
                SetLookRotation(kccFixedData, input.LookRotationDelta, _weapons.GetRecoil(), out newRecoil);
            }
		...
		private void ProcessRenderInput()
		{
            ...
            if (_health.IsAlive == true)
			{
                ...
                GameplayInput cachedInput = _agentInput.CachedInput;
